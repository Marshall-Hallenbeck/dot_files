...
- Under WSL, `command -v <tool>` can resolve to a Windows npm global shim (`/mnt/c/Users/<u>/AppData/Roaming/npm/<tool>`). Those shims are extensionless `/bin/sh` scripts that `exec node`, so they are found but fail with `exec: node: not found` — Windows ships `node.exe`, which bare `node` never matches. Two consequences: (1) `if ! command -v X` install guards see a "present" tool and skip the Linux install permanently; (2) the broken shim wins at runtime if the Windows npm dir precedes the Linux bin dirs in PATH. Fix guards by rejecting `/mnt/*` resolutions, and put `$HOME/.local/bin` *before* the inherited `$PATH`, not after. Only extensionless shims leak through this way (npm's `codex`, `ruler`, `yarn`), which is why `.exe`-only tools never collide.
- npm CLIs installed under nvm are unreachable from systemd user units: their shebang is `#!/usr/bin/env node`, and the systemd user PATH is `/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin` with no nvm. Symlinking `$(nvm which default)` to `/usr/local/bin/node` fixes every such CLI at once. Verify with `env -i HOME=$HOME PATH=<systemd PATH> <tool> --version`, not from an interactive shell — an interactive shell has nvm loaded and hides the bug.
...
- In `find -path`, `*` also matches `/`, so `-path '*/vendor/*/bin'` matches a `bin` directory at ANY depth below `vendor/`. Paired with `-print -quit` it silently returns whichever match the depth-first walk reaches first, so a package that later adds a deeper `bin` (the Codex npm package added a vendored zsh at `vendor/<triple>/codex-resources/zsh/bin`) flips the answer and the script links the wrong directory. Prefer a shell glob, which never crosses `/`, for fixed-depth paths: `pkg/*/vendor/*/bin/codex`. Note `find` in a Claude Code shell is a `bfs` wrapper (breadth-first) from the shell snapshot, so an interactive check can return the shallow, correct match while the same command in a `#!/bin/bash` script uses GNU find and returns the deep, wrong one.
...
- A published host port that is already taken can leave a container **running but with no network endpoint**: `docker run`/`compose up` fails with `driver failed programming external connectivity ... address already in use`, yet a retry starts the process while the network attach stays undone. Symptom is a sibling container failing DNS on the service name (`lookup <svc>: i/o timeout`) while `docker ps` shows the target `Up (healthy)` with an empty PORTS column. Confirm with `docker inspect <c> --format '{{range $k,$v := .NetworkSettings.Networks}}{{$k}} {{$v.IPAddress}}{{end}}'` — empty output means detached. Fix is `docker compose down && up -d` after freeing the port; named volumes survive a `down` without `-v`.
