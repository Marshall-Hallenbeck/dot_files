...
Reads an implementation plan and executes it via Opus 4.6 Max Effort  parallel subagents, with integration testing and regression tracking.
...
For each track, spawn an Opus 4.6 Max Effort subagent via Task tool with:
...
Use the project's detected test runner (pytest, jest, vitest, cargo test, go test). See `/run-unit-tests` for runner detection.
